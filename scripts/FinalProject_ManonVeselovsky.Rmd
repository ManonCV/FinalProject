...
``` {r}
m = glmer(roost ~ distance + sex + ordinal_date + sex:ordinal_date + moon.phase + (1|colony/id), data=depart_final, family="binomial",na.action=na.omit)
```


``` {r}
m = glmer(roost ~ distance + sex + ordinal_date + sex:ordinal_date + moon.phase + (1|colony/id), data=df.scale, family="binomial",na.action=na.omit)


#examine
checked = check_model(m)
checked

# performance::binned_residuals(m, term='ordinal_date')
# performance::binned_residuals(m, term='distance')
# performance::binned_residuals(m, term='moon.phase')
...
plot(dharm_output)
plotResiduals(dharm_output, form=df.scale$distance) #some overdispersion but not bad, indicates possible non-linear relationship with distance
plotResiduals(dharm_output, form=df.scale$ordinal_date)
plotResiduals(dharm_output, form=df.scale$sex)
plotResiduals(dharm_output, form=df.scale$id)
plotResiduals(dharm_output, form=df.scale$colony)

#see if transformation to distance helps fit - appears to be a -ve exponential relationship from ggplot in section 1.2
...
dharm_output_neg = simulateResiduals(fittedModel=m_neg,plot = F)
plotResiduals(dharm_output_neg, form=df.scale$distance) 
...
dharm_output = simulateResiduals(fittedModel=m_exp,plot = F)
plotResiduals(dharm_output_exp, form=df.scale$distance) 
...
plotResiduals(simulateResiduals(fittedModel=m_sqrd,plot = F), form=df.scale$distance)
