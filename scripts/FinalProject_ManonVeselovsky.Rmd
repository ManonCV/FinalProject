...
#remove rows with NA for distance
nas_dropped = depart_final
nas_dropped = nas_dropped[complete.cases(df[ , c('distance')])]
...
depart_transformed = depart_final
depart_transformed$ordinal_date = exp(depart_final$ordinal_date)

m_trans = glmer(roost ~ distance + sex + ordinal_date + sex:ordinal_date + moon.phase + (1|colony/id), data=depart_transformed, family="binomial")
...
plot(dharm_output)
plotResiduals(dharm_output, form="distance")
