...
scale_orddate = scale(depart_final$ordinal_date)
scaleList_orddate = list(scale = attr(scale_dist, "scaled:scale"),
    center = attr(scale_dist, "scaled:center"))
...
plotResiduals(simulateResiduals(fittedModel=m_exp,plot = F), form=df.scale$distance) #fit not much better
...
plotResiduals(simulateResiduals(fittedModel=m_neg,plot = F), form=df.scale$distance) #fit worse
...
effect_date = as.data.frame(effects_m$ordinal_date)
effect_orddate_unscaled = as.data.frame(effects_m$ordinal_date)
effect_ordsex_unscaled$'sex:ordinal_date' = effect_ordsex$'sex:ordinal_date'*scaleList_orddate$scale + scaleList_orddate$center
effect_ordsex_unscaled



mean(depart_final$distance)
scaleList
print(distance_effect_unscaled)
effects_m
table_effects = summary(m)
str(table_effects)
