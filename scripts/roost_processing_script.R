...
setwd("C:/Users/Manon/Documents/BIO8940 - Advanced Statistics/FinalProject")
...
library(GeoLight) #sunrise and sunset times
library(RNCEP)
library(glmmADMB) #download from source, use GLMMTMB instead
# require(devtools)
...
# detections <- readRDS("Banding_Data_2015_adults_Only.rds")
# 
...
banding <- read.csv("data/Banding_Data_2015_adults_Only.csv")
...
colony_locations <- read.csv("data/colony_locations.csv")
...
evenings <- read.csv("data/evening_data.csv")
...
morning <- read.csv("data/morning_data.csv")
...
write.csv(departure, file= "departure_dat.csv")
...
model1 <- glmer(roost ~ distance + sex + ordinal_date + tailwind +  (1|id), data=departure, family="binomial")
